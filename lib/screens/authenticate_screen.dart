...
class _AuthenticateScreenState extends State<AuthenticateScreen> {
  ...
  _fetchUsersAndMatchFace() async {
    ...
    if (box.isNotEmpty) {
      users.clear();
      log(box.length.toString(), name: "Total Registered Users");

      for (int i = 0; i < box.length; i++) {
        UserModel user = box.getAt(i)!;
        double similarity = compareFaces(_faceFeatures!, user.faceFeatures  ?? FaceFeatures());
  ...
  _fetchUserByName(String name) async {
    var box = Hive.box<UserModel>('users');

    var matchedUser = box.values.where((user) => user.name == name).toList();
    if (matchedUser.isNotEmpty) {
      users.clear();
      setState(() {
        users.add([matchedUser.first, 1]);
      });
      _matchFaces();
    } else {
      setState(() => trialNumber = 1);
      _showFailureDialog(
        ...
        description: "User is not registered yet. Register first to authenticate.",
      );
    }
  }
